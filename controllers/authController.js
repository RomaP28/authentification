...
exports.signup = catchAsync(async (req, res, next) => {
    ...
    //get user data from request and store in variable newUser
...
exports.login = catchAsync(async (req,res,next) => {
    ...
    // 2) Check if user and password is correct ('select' for showing hidden password)
...
exports.protect = catchAsync(async (req, res, next)=> {
    ...
    // 4) Check if changed password after the token was issued (if user change password after getting token)
...
exports.isLoggedIn = async (req, res, next)=> {
    if (req.cookies.jwt) {
        try {
            ...
            // 3) Check if changed password after the token was issued (if user change password after getting token)
            if (currentUser.changedPasswordAfter(decoded.iat)) return next()

            //There is a logged in user
            //locals it is global place for variable for pug templates
            res.locals.user = currentUser;
            return next();
        } catch (err) {
            return next();
...
//Logined user want update password
